...
消费者订阅topic,对topic的信息按照一定的顺序(从最早,或最近或存的offset)做出反应.
这样每个消费者对应的消息队列中就只包含自己想要的.  
每个消费者对应一个offset来记录当前消费的消息位置.即使发生系统宕机,重新恢复后也可以从offset处重新开始.


# Parition和Replica  
Topic内部,数据会分区(partition),分区的编号从0开始.每个分区都有副本(replica),副本的数量是可以设置的.


# Broker集群
为了提高容错和速度,kafka使用中间件集群.分区的副本存在不同的中间价中,每个分区都有一个leader和一个或多个follower,
leader负责提供读写服务,follower同步leader数据.  
leader坏了就用follower.这样做也提高了吞吐量,因为消费者可以在follower中进行查询.
